...
class Laser():
    ...
    def draw(self,window):
        window.blit(self.img,(self.x,self.y,)) # Ucundan Çıkmasını ayarladık
...
class PlayerShip(Ship):
    ...
    def shoot(self):
        if self.cool_down_counter==0:
            laser=Laser(self.x+20,self.y,self.lazer_img) # PlayerShip İLe Enemyship e ekleiyp nereden cıkcagını ekledık ates topunun
            self.lasers.append(laser)
...
def main():
    ...
    while run:
        ...
        for enemy in enemies: # BURDA DÖNGÜYE SOKARAK DÜŞMAN GEMİLERİ AŞAĞIYA GETİRDİK
            ...
            enemy.MoveLasers(laser_velocity)
